...
window.salvarMensagem = () => {
    ...
    push(ref(db, "mensagens"), { nome, texto: input.value, hora, resp: respondendoA });
    input.value = "";
    cancelarResposta();
};
...
onValue(ref(db, "mensagens"), (snapshot) => {
    ...
    snapshot.forEach((child) => {
        ...
        let cita = d.resp ? `<div style="background:rgba(0,0,0,0.1); padding:5px; border-left:3px solid var(--verde); border-radius:4px; font-size:0.7rem; margin-bottom:5px;"><b>${d.resp.u}</b>: ${d.resp.t}</div>` : "";

        div.innerHTML = `
            <span class="msg-name" onclick="prepararResposta('${d.nome}', '${d.texto}')">${d.nome}</span>
            ${cita}
...
// --- AVISOS ---
window.salvarAviso = () => {
    const s = prompt("Acesso Administrativo:");
    if(s === "Chernobyl") {
